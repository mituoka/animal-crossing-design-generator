...
def quantize_colors(image: Image.Image, palette_size: int) -> Tuple[Image.Image, List[Tuple[int, int, int]]]:
...
def generate_pixel_art(
    ...
    if input_path and os.path.exists(input_path):
        ...
        if "flower" in input_text.lower():
            ...
            center = size // 2
            radius = size // 4
            colors = [(255, 0, 0, 255), (255, 255, 0, 255), (0, 255, 0, 255), (0, 0, 255, 255), (255, 0, 255, 255)]
            ...
            draw.ellipse([center - radius//2, center - radius//2, center + radius//2, center + radius//2], fill=(255, 255, 0, 255))
            ...
            # 家本体
            draw.rectangle([margin, margin + roof_height, margin + house_width, margin + house_width], fill=(150, 75, 0, 255))
            ...
            draw.rectangle([door_x, door_y, door_x + door_width, door_y + door_height], fill=(100, 50, 0, 255))
        ...
        design_data = {
            ...
            "height": size,
            "palette": [{"r": c[0], "g": c[1], "b": c[2]} for c in palette],
            "pixels": [{"x": i % size, "y": i // size, "color_index": palette.index((p[0], p[1], p[2])) if p[3] > 128 else -1} for i, p in enumerate(pixel_data)]
        }
